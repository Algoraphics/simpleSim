				...
				if (data->wrenchReceived > 0 && time_diff != current_time) {
					...
					if (data->pdState == JAUS_READY_STATE) {
						if (data->gear != DRIVE) {
							...
							// steering control
							double delta_toss;
							steering_controller(&omega, &delta_toss, &path_index, x, y, velocity, heading, tire_angle, data->path_x, data->path_y, 0);
							...
							// comfort control
							comfort_controller(&acceleration, path_index, x, y, velocity, heading, tire_angle, data->path_x, data->path_y, data->lod_x, data->lod_y);
...
void VehicleController::steering_controller(double *omega, double *delta_d, int *path_index, double x, double y, double velocity, double heading, double tire_angle, double *path_x, double *path_y, int flag) {
	double om = *omega;
...
void VehicleController::comfort_controller(double *acceleration, int path_index, double x, double y, double velocity, double heading, double tire_angle, double *path_x, double *path_y, double *lod_x, double *lod_y) {
	...
	if (look_ahead < n) {
		...
		steering_controller(&omega, &delta_d, &path_index_placeholder, x, y, velocity, heading, tire_angle, path_x, path_y, 1);
	} else {
