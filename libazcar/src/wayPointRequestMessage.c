...
static const int commandCode = JAUS_EXPERIMENTAL_WAY_POINT_REQUEST_DATA;
static const int maxDataSizeBytes = 0;

static JausBoolean headerFromBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes);
static JausBoolean headerToBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes);
static int headerToString(WayPointRequestMessage message, char **buf);

static JausBoolean dataFromBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes);
static int dataToBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes);
static void dataInitialize(WayPointRequestMessage message);
static void dataDestroy(WayPointRequestMessage message);
static unsigned int dataSize(WayPointRequestMessage message);
...
static void dataInitialize(WayPointRequestMessage message) {
	...
	// SW: hoping I can do this with a user defined struct.
	message->wayPoint = wayPointCreate();
}

// Destructs the message-specific fields
static void dataDestroy(WayPointRequestMessage message) {
	// trash the way point
	wayPointDestroy(message->wayPoint);
}

// Return boolean of success
static JausBoolean dataFromBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes) {
	WayPoint tempObject = wayPointCreate();

	if (bufferSizeBytes == dataSize(message)) {
		// use the wayPoint from buffer function
		if (!wayPointFromBuffer(&tempObject, buffer, bufferSizeBytes)) {
			printf("Error retrieving way point from buffer!!\n");
			return JAUS_FALSE;
		}
		dataDestroy(message);
		message->wayPoint = tempObject;
		return JAUS_TRUE;

	} else {
		printf("Buffer size not appropriate in dataFromBuffer(WayPointRequestMessage...)\n");
		printf("Leaving dataFromBuffer(WayPointRequestMessage...) with JAUS_FALSE\n");
		return JAUS_FALSE;
	}
}
...
static int dataToBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes) {
	...
	if (bufferSizeBytes >= dataSize(message)) {
		if (!wayPointToBuffer(message->wayPoint, buffer, bufferSizeBytes)) {
			printf("Error placing way point message into buffer!!\n");
			return JAUS_FALSE;
		}
		index += wayPointSize(message->wayPoint);

	} else {
		// bad things...
		printf("Bad things in dataToBuffer(WayPointRequestMessage...)\n");
	}
...
static int dataToString(WayPointRequestMessage message, char **buf) {
	//Assume: message already verified

	int bufSize = 1024;
	char *tempStr = (char *)malloc(sizeof(char) * bufSize);
	sprintf(tempStr, "\nWay Point Request (%s)", wayPointToString(message->wayPoint));
...
// Returns number of bytes put into the buffer
static unsigned int dataSize(WayPointRequestMessage message) {
	return wayPointSize(message->wayPoint);
}
...
WayPointRequestMessage wayPointRequestMessageCreate(void) {
	WayPointRequestMessage message;

	message = (WayPointRequestMessage)malloc(sizeof(WayPointRequestMessageStruct));
	if (message == NULL) {
		return NULL;
	}
...
void wayPointRequestMessageDestroy(WayPointRequestMessage message) {
	dataDestroy(message);
...
JausBoolean wayPointRequestMessageFromBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes) {
	int index = 0;
...
JausBoolean wayPointRequestMessageToBuffer(WayPointRequestMessage message, unsigned char *buffer, unsigned int bufferSizeBytes) {
	if (bufferSizeBytes < wayPointRequestMessageSize(message)) {
		...
		if (headerToBuffer(message, buffer, bufferSizeBytes)) {
			...
			return JAUS_FALSE; // headerToWayPointRequestBuffer failed
		}
	}
}
...
char *wayPointRequestMessageToString(WayPointRequestMessage message) {
	if (message) {
		...
		char *buf = "Invalid WayPointRequest Message";
